...
import { NHKAPI, Notification } from "./type.ts";
...
export async function reqGetNotification(): Promise<Notification> {
  const res = await fetch(
    `${baseUri}/config/notification`,
    {
      headers: {
        "Authorization": `Bearer ${token}`,
      },
...
export async function reqPostNotification(reqBody: Notification) {
  const res = await fetch(
    `${baseUri}/config/notification`,
    {
      method: "POST",
